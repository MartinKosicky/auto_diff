...
namespace AD {
	...
	Utils::matrix Utils::CreateMatrix(std::vector<std::vector<float> >&&v)
	{
		size_t prevSize = 0;
		for (size_t i = 0; i < v.size(); i++)
		{
			assert(v[i].size() != 0 && (i == 0 || (v[i].size() == prevSize)));  // all equal size
			prevSize = v[i].size();
		}

		matrix newMatrix(v.size(), prevSize);
		for (size_t i = 0; i < v.size(); i++)
		{
	...
	Utils::matrix Utils::CreateRowMatrix(std::vector<float>&& v)
	{
		matrix newMatrix(1, v.size());
		for (size_t i = 0; i < v.size(); i++)
			newMatrix(0, i) = v[i];
		return newMatrix;
	}
